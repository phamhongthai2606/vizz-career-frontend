...
const FeaturedJobsSection: React.FC<FeaturedJobsSectionProps> = ({
  ...
  const JOBS_PER_SHOW_MORE = 4; // số job mỗi lần click show more
  ...
  // === SPLIT LIST ===
  const sliderJobs = jobsFeatured.slice(0, SLIDER_COUNT);
  const remainingJobs = jobsFeatured.slice(SLIDER_COUNT);

  // === FIRST ROW ===
  const firstRowJobs = sliderJobs.slice(0, jobsPerRowResponsive);

  // === SHOW MORE JOBS ===
  let showMoreJobs: typeof jobsFeatured = [];
  ...
  return (
    <section className="bg-gray-100 px-[60px] py-20 xl:px-[120px]">
      <div className="flex flex-col items-center">
        ...
        {/* Show More Button */}
        {firstRowJobs.length + showMoreJobs.length < jobsFeatured.length && (
