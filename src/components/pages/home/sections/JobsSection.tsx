...
const JobsSection: React.FC<JobsSectionProps> = ({
  ...
  // === SPLIT LIST ===
  const sliderJobs = jobs.slice(0, SLIDER_COUNT);
  const remainingJobs = jobs.slice(SLIDER_COUNT);

  // === FIRST ROW ===
  const firstRowJobs = sliderJobs.slice(0, jobsPerRowResponsive);

  // === SHOW MORE JOBS ===
  let showMoreJobs: typeof jobs = [];
  ...
  return (
    <section className="bg-gray-100 px-[60px] py-20 xl:px-[120px]">
      <div className="flex flex-col items-center">
        ...
        {/* Show More Button */}
        {firstRowJobs.length + showMoreJobs.length < jobs.length && (
